...
entry migration v14 closes all four gates—even for an opted-in Home.20 entry with all four
...
Home.21 retains Home.14's privacy-safe basket failure classification and Home.15's durable
...
Home.21 closes the complete evidence-backed public basket-schema matrix in one batch.
Bounded unknown root, lifecycle/store, summary, product, price, MBS, packaging, weighable,
quantity-display, and customization-display keys are stripped like ordinary Zod objects.
Known display strings are preserved byte-for-byte and declared display numbers accept finite
JSON numbers with their public nullability. Summary string distance, delivery-fee display
objects, and finite-number ETA objects are accepted and discarded without becoming adoption
authority. Product suggestions are bounded display-only input and are discarded wholesale,
so suggestion drift cannot block an otherwise exact basket.

Customization `groupLegacyId` and `groupId` are independent. Display labels (`name`,
`groupName`, and `customizationName`) are independent and do not participate in exact intent
matching. Group/option IDs, external IDs, position, and quantity remain exact. Optional
product and replacement `legacyId: null` is treated as absence. Repeated optional product
enrichment IDs are accepted, while authoritative product mapping and basket-line IDs remain
unique. Empty or sparse discovered baskets cannot authorize adoption and become `CONFLICT`.

The three discount fields declared nullable by Glovo remain accepted and preserved on
basket products; malformed non-null neighbors still fail. Product
`quantity.incrementsLimit` accepts a nonnegative signed-32-bit provider ceiling independently
of the local hard quantity cap; zero and ceilings above 50 normalize to no effective limit
inside the closed 1..50 domain. Negative, string, boolean, fractional, and overflow limits
remain rejected. Customer, store, address, category, basket/version, product,
`basketProductId`, customization identity/position/quantity, selected quantity, and final
minor-unit authority remain strictly bound to the exact prepared intent. The global payload,
depth, cardinality, product, customization, and total-quantity resource bounds are unchanged.
...
Home.21 keeps `ORDERING_WEB_VERSION` at `v1.2569.0`, the latest reproducibly preserved
...
Home.21 is offline release readiness only: it does not claim deployment, a live canary,
