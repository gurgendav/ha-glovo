...
mutation; Home.18 does not inherit its consent or writable basket authority.
...
`ORDERING_WEB_VERSION` for Home.18. No authenticated or Glovo API request was made.
...
Home.18 implements only the bounded read policy justified by those two routes: one
...
added privacy-safe failure classification, and Home.18 retains both behaviors. The strict
...
validation, Home.14 added privacy-safe failure classification, and Home.18 retains both.
...
nullable. Home.18 therefore accepts exactly `null` or the pre-existing bounded string
...
other evidence-backed compatibility relaxation. Home.18 accepts a nonnegative signed
...
1. migration v11 closes all four gates—including for fully opted-in Home.17 entries—and requires a fresh default-off administrator re-opt-in;
...
This evidence does **not** establish provider idempotency or authorize a payment. Home.18 only composes the reviewed strict adapter behind fresh default-off gates and healthy durable authority; it preserves the no-retry policy and blocks every later checkout until an ambiguous outcome is manually or provider-status reconciled.
