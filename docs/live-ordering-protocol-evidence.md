...
mutation; Home.19 does not inherit its consent or writable basket authority.
...
`ORDERING_WEB_VERSION` for Home.19. No authenticated or Glovo API request was made.
...
Home.19 implements only the bounded read policy justified by those two routes: one
...
added privacy-safe failure classification, and Home.19 retains both behaviors. The strict
...
validation, Home.14 added privacy-safe failure classification, and Home.19 retains both.
...
nullable. Home.19 therefore accepts exactly `null` or the pre-existing bounded string
...
other evidence-backed compatibility relaxation. Home.19 accepts a nonnegative signed
...
`storeProductId` enrichment values. Home.19 accepts those repetitions while retaining
...
1. migration v12 closes all four gates—including for fully opted-in Home.18 entries—and requires a fresh default-off administrator re-opt-in;
...
This evidence does **not** establish provider idempotency or authorize a payment. Home.19 only composes the reviewed strict adapter behind fresh default-off gates and healthy durable authority; it preserves the no-retry policy and blocks every later checkout until an ambiguous outcome is manually or provider-status reconciled.
